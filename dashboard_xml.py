...
class PortfolioDashboard:
    ...
    def export_to_excel(self, modules: list = None):
        """Exporte les données dans le modèle Excel par modification OOXML."""
        ...
        print(f"⏳ Export Excel (OOXML) démarré → {os.path.basename(self.path_output)}")
        wb = OoxmlWorkbook(self.paths["wb_input"])
    ...
    def _export_fonds(self, ws):
        ...
        try:
            data_range.api.HorizontalAlignment = "center"
        except Exception:
            pass
    ...
    def _export_analyse_2(self, ws):
        """Insère les images VL (B2) et Sentiment (Q2) dans le package OOXML."""
